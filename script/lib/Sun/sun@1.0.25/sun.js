 ...
 *      + v1.0.23 ：增加encodeBase64、decodeBase64
 *
...
Sun.Util.Common = {
    ...
    forbidConsole : function (url){
        document.addEventListener('contextmenu', function(event){
            return event.returnValue = false
        })
        Sun.defendInterval = setInterval(function (){
            var element = new Image();
            Object.defineProperty(element,'id',{get:function(){window.location.href=url||"http://www.8net.com.cn/"}});
            console.log(element);
        },100);
    },
 ...
 * Update Note：
 *      + v1.0.0 ：Created
 *      + v1.0.24 ：增加获取canvas填充的pattern的方法getCanvasPattern
 *
...
Sun.Util.Color = {
    ...
    patternFn: {
        'r_bias':function (ctx,a) {
            this.drawLine(ctx,0,a,a,0);
            this.drawLine(ctx,-1,1,1,-1);
            this.drawLine(ctx,a-1,a+1,a+1,a-1);
        },
        'l_bias':function (ctx,a) {
            this.drawLine(ctx,0,0,a,a);
            this.drawLine(ctx,-1,a-1,1,a+1);
            this.drawLine(ctx,a-1,-1,a+1,1);
        },
        'h_line': function(ctx,a){
            this.drawLine(ctx,0,a/2,a,a/2);
        },
        'v_line': function(ctx,a){
            this.drawLine(ctx,a/2,0,a/2,a);
        },
    ...
    /**
     * 获取canvas填充的pattern
     * @param type {string} r_bias/l_bias/h_line/v_line
     * @param color {string} 填充线的颜色
     * @param width {int} 单位格宽度
     * @param lineWidth {int} 填充线线宽
     * @param fill {false|'#fff'} 无填色或者指定的颜色值
     * @returns {CanvasPattern}
     */
    getCanvasPattern:function (type,color,width,lineWidth,fill){
        ...
        Sun.Color.patternFn[type](ctx,width);
 ...
 *      + v1.0.23 : 1. 增加transformGridLatOrder方法，将网格数据的纬度存储顺序转向
 *                  2. changeGridNcToJson增加nc3.0的variable增加terrain字段和单个variable增加精度的解析
 *                  3. micaps 14类文件增加一些字段的解析
 *
 * @class Sun.Util.Data
 */
Sun.Util.Data = {
    ...
    changeGridNcToJson:function (nc,windType,precision) {
        ...
        function setData2() {
            ...
            if(data.version === '3.0'){
                ...
                data.xaxis = getDataVariable('XAxis');
                // data.xaxis = ncReader.getDataVariable('XTime',data.precision);
                data.yaxis = getDataVariable('YAxis');
                data.xaxis_uv = getDataVariable('xAxis_uv');
                data.yaxis_uv = getDataVariable('YUV');
                data.terrain = getDataVariable('terrain');
                data.hpa_km = [
                    getDataVariable('YhPa'),
                    getDataVariable('YKm')
                ]
            }

            function getDataVariable(code){
                var prcs = ncReader.getVariableAttribute(code,'Precision') || data.precision;
                return ncReader.getDataVariable(code,prcs);;
            }
        }
    ...
    changeGridInt16ToJson:function(bin){
        ...
        function setDataJson(data) {
            if (data.GridType) {
                ...
                var pcs = data.DataPrecision,invalid = data.invalidValue;
                if (data.GridType == 11)
                    grid = [[], []];
                for (var i = 0; i < gridSize; i++) {
                    if (data.GridType == 4){
                        var value = data.data[i]
                        grid[i] = value == invalid ? value : value / pcs;
                    }
                    else if (data.GridType == 11) {//Tip:11类uv风数据
                        var u = data.data[0][i], v = data.data[1][i];
                        grid[0][i] = u == invalid ? u : u / pcs;
                        grid[1][i] = v == invalid ? v : v / pcs;
                    }
                }
                data.data = grid;
            }
    ...
    changeMicapsToJson:function (micaps) {
        ...
        if (micapsType == '4') {//4类文件
            for (json.data = []; idx < data.length; idx++) {
                json.data.push(parseFloat(data[idx]));
            }
        }
        else if (micapsType == '11') {//11类文件
            var size = idx + json.latsize * json.lonsize;
            for (json.data = [[], []]; idx < data.length; idx++) {
                var item = parseFloat(data[idx]);
                idx < size ? json.data[0].push(item) : json.data[1].push(item);
            }
        }
    ...
    changeMicaps14ToJson:function (micaps) {
        ...
        for (json.lines = [],idx++,i=0,count=parseInt(data[idx++]); i < count; i++) {
            json.lines.push(getIsoline(['lineWidth']));
        }
        // 标志线
        for (json.symbolLines = [],idx++,i=0,count=parseInt(data[idx++]); i < count; i++) {
            // var code = data[idx++];
            var lines = getIsoline(['code','lineWidth']);
            // lines.code = code;
        ...
        for (json.symbols = [],idx++,i=0,count=parseInt(data[idx++]); i < count; i++) {
            code = data[idx++];
            var x = data[idx++],y = data[idx++],z = data[idx++],str=data[idx++];
            json.symbols.push({code:code,latlng:[y,x],str:str});
        }
        // 闭合面
        for (json.planes = [],idx++,i=0,count=parseInt(data[idx++]);  i < count; i++) {
            json.planes.push(getIsoline(['lineWidth']));
        }
        // 站点
        json.stations = getStation();
        // 天气区
        for (json.weatherRegion = [],idx++,i=0,count=parseInt(data[idx++]);  i < count; i++) {
            // TODO:
        }
        // 填充区
        for (json.fillArea = [],idx++,i=0,count=parseInt(data[idx++]);  i < count; i++) {
            // TODO:
        }
        // 标注
        for (json.notesSymbol = [],idx++,i=0,count=parseInt(data[idx++]);  i < count; i++) {
            // TODO:
        }
        // 带属性的标志线
        for (json.symbolLines_withProp = [],idx++,i=0,count=parseInt(data[idx++]);  i < count; i++) {
            json.symbolLines_withProp.push(getIsoline(['code','lineWidth','a','r','g','b','lineType','shadow']));
        }

        return json;

        function getIsoline(props,noPoint) {
            var lines = {};
            for(var i=0;i<props.length;i++){
                lines[props[i]] = data[idx++];
            }
            lines.data = getLatlngs();
            var point = {};
            if(!noPoint){
                var label = data[idx++];
                points = {value:label=='NoLabel'?label:parseFloat(label),data:getLatlngs()};
            }
     ...
     * @param data
     */
    transformGridLatOrder:function (data) {
        var x = data.lonsize, y = data.latsize;
        var grids = [];
        for(var i=0,j=0;i<y;i++,j+=x){
            var item = data.data.slice(j,j+x);
            grids.push(item);
        }
        data.data = grids.reverse().join(',').split(',');
    }
};
 ...
 *     + v1.0.23 : 1. 增加绘制扇形的功能drawSector
 *                 2. 增加绘制浪向箭羽的功能drawArrowFeather
 *                 3. 设置多边形的空间关系的方法setPolygonRelations
 *     + v1.0.24 : 1. 风杆绘制增加描边功能
 *     + v1.0.25 : 1. 集成风杆、箭头、箭羽旋转中心点的计算,直接配置boolean类型的参数
 *
...
Sun.Util.Geometry = {
     ...
     * @param p {object} 绘制位置
     * @param fill {boolean} 风杆三角形部分是否填色
     * @param xHeight {number} 风杆长度
     * @param xWidth {number} 风杆宽度
     * @param centerRotate {boolean} [optional] 是否按风羽的中心点旋转
     * @param stroke {boolean|string} eg:false/'#fff'
     * @param strokeWidth {int} 描边宽度 default 3
     */
    drawWindByPosition: function (ctx, windSpeed, windDir, p, fill, xHeight, xWidth, centerRotate, stroke, strokeWidth) {
        if (!ctx) return;
        if(windSpeed!=null && windSpeed>0) {
            var x = p.x;
            var y = p.y;
            ...
            ctx.beginPath();
            ctx.moveTo(x, y);
            ctx.lineTo(x, y - xHeight);

            for (var i = 0; i < wholeLines; i++) {
                ctx.moveTo(x, y - xHeight);
                if (_wholeLines >= 5) {
                    ctx.lineTo(x + xShift, y - xHeight);
                    ctx.lineTo(x, y - xHeight + yShift);
                    if (fill)
                        ctx.fill();
                    i += 4;
                    _wholeLines -= 5;
                    xHeight -= (_wholeLines>=5 ? yShift : 2*yShift);
                } else {
                    ctx.lineTo(x + xShift, y - xHeight - yShift);
                    xHeight -= interval;
                }
            ...
            if (halfLine) {
                if (wholeLines == 0) xHeight -= interval;
                ctx.moveTo(x, y - xHeight);
                ctx.lineTo(x + xShift / 2 + 1, y - xHeight - yShift / 2);
            }

            if(stroke){//Tip:是否描边
                var s_strokeStyle = ctx.strokeStyle;
                var s_lineWidth = ctx.lineWidth;
                ctx.strokeStyle = stroke;
                ctx.lineWidth = strokeWidth || 3;
                ctx.stroke();
                ctx.strokeStyle = s_strokeStyle;
                ctx.lineWidth = s_lineWidth;
            }
            ctx.stroke();
            ctx.setTransform(Sun.Common.dpr, 0, 0, Sun.Common.dpr, 0, 0);
        }
     ...
     * @param theta 箭头角度
     * @param headlen 箭头长度
     * @param centerRotate {boolean} [optional] 是否按箭头的中心点旋转
     */
    drawArrow:function (ctx, p, dir, vaneLength,theta,headlen,centerRotate) {
        vaneLength = typeof(vaneLength) != 'undefined' ? vaneLength : 25;
        var tx = p.x, ty = p.y;
        if(centerRotate){
            p.y = p.y+vaneLength/2;
        }
        var p2 = {x:p.x,y:p.y-vaneLength};
        ctx.translate(tx, ty);
        ctx.rotate(dir * Math.PI / 180);
        ctx.translate(-tx, -ty);
        Sun.Util.Geometry.drawArrowByXY(ctx,p.x,p.y,p2.x,p2.y,theta,headlen);
        ctx.setTransform(Sun.Common.dpr, 0, 0, Sun.Common.dpr, 0, 0);
    },

    drawArrowByXY: function (ctx, fromX, fromY, toX, toY,theta,headlen) {
        theta = Sun.Common.isValid(theta) ? theta : 30;
        headlen = Sun.Common.isValid(headlen) ? headlen : 6;
     ...
     * @param theta 箭头角度
     * @param headlen 箭头长度
     * @param centerRotate {boolean} [optional] 是否按箭羽的中心点旋转
     */
    drawArrowFeather:function(ctx,p,dir,period,xHeight,xWidth,theta,headlen,centerRotate){
        xHeight = Sun.Common.isValid(xHeight) ? xHeight : 25;
        var tx = p.x, ty = p.y;
        if(centerRotate){
            p.x = p.x+xWidth/2;//Tip:风羽往右绘制，箭羽往左绘制，所以一加一减
            p.y = p.y+xHeight/2;
        }
        var p2 = {x:p.x,y:p.y-xHeight};
        ctx.translate(tx, ty);
        ctx.rotate(dir * Math.PI / 180);
        ctx.translate(-tx, -ty);
        drawFeather();
        ...
        function drawFeather() {
            var wholeLines = Math.floor(period / 2);
            var halfLine = ((Math.floor(period) % 2) > 0);
            var carriage = xHeight;
            var interval = xWidth/2;
            var wholeLines;
            var x = p.x,y = p.y+10;
            for (var i = 0; i < wholeLines; i++) {
                ctx.moveTo(x, y - carriage);
                ctx.lineTo(x - xWidth, y - carriage);
                carriage -= interval;
            }

            if (halfLine) {
                ctx.moveTo(x, y - carriage);
                ctx.lineTo(x - xWidth / 2 -1, y - carriage);
            }
            ctx.stroke();
        }
    },
     ...
     * @return {*}
     */
    drawSector:function(ctx,x,y,r,angle1,angle2) {
        angle1 = angle1-90;
        angle2 = angle2-90;
        ctx.save();
        ctx.beginPath();
    ...
    /**
     * 设置多边形的空间关系，并写入pid,cid。用latlngInPolygon
     * @param data
     * @param field {String} 经纬度的字段名
     */
    setPolygonRelations:function (data,field){
        for (var i = 0; i < data.length; i++) {
            var line0 = data[i];
            var p0 = line0[field][0];
            line0.cid = line0.cid || [];
            for (var j = i + 1; j < data.length; j++) {
                var line1 = data[j];
                var p1 = line1[field][0];
                line0.id = i;
                line1.id = j;
                line1.cid = line1.cid || [];
                if (Sun.Util.Geometry.latlngInPolygon(p0, line1[field])) {
                    // line0在line1里面
                    if (isNaN(line0.pid)) {
                        line0.pid = j;
                        line1.cid.push(i);
                        ...
                        if (Sun.Util.Geometry.latlngInPolygon(p1, line2[field])) {
                ...
                else if (Sun.Util.Geometry.latlngInPolygon(p1, line0[field])) {
                    if (isNaN(line1.pid)) {
                        line1.pid = i;
                        line0.cid.push(j);
                    }
                        ...
                        if (Sun.Util.Geometry.latlngInPolygon(p0, line2[field])) {
 ...
 * Update Note：
 *      + v1.0.15 : Created
 *      + v1.0.23 : 增加getIdxAndLen-根据独立索引转换成连续索引的描述
 *      + v1.0.24 : 增加getGridOfStations-根据站点数据获取网格数据
 *
 * @class Sun.Util.Grid
 */
Sun.Util.Grid={
    ...
    /**
     * 根据独立索引转换成连续索引的描述,仅针对单网格
     * @param indexes {Array} eg:["46_179", "46_180", "46_181"]
     * @returns {string}
     */
    getIdxAndLen: function (gridModel,indexes){
        var idxAndLen = [],idx=-1,count=0;
        for(var i=0;i<indexes.length;i++){
            var item = indexes[i].split('_');
            if(idx != item[0]){
                idx = item[0];
                if(count!=0)
                    idxAndLen.push(count);
                idxAndLen.push(parseInt(item[1])+parseInt(item[0])*gridModel.data.lonsize);
                count=1;
            }
            else
                count++;
     ...
     *     options.rows {int}
     *     options.column {int}
     *     options pow {int} default 5
     *     options.iPointMinNum {int} 参与插值的离散点最少数量。default 3
     * }
     * @param stationOptions {Object} {
     *     options.valueField {string}
     *     options.valueScale {string} default: 1
     *     options.latField {string} default: 'STATIONLAT'
     *     options.lonField {string} default: 'STATIONLON'
     * }
     */
    getGridOfStations:function (data,options,stationOptions){
        var grid = [];
        var rows = options.rows, columns = options.columns,
            pow = options.pow || 5,iPointMinNum = options.iPointMinNum || 3;
        var latField = stationOptions.latField || 'STATIONLAT',
            lonField = stationOptions.lonField || 'STATIONLON',
            valueScale = stationOptions.valueScale || 1,valueField = stationOptions.valueField;
        var bounds = options.bounds;
        var nlat = (bounds.getNorth() - bounds.getSouth())/(rows - 1);
        var nlon = (bounds.getEast() - bounds.getWest())/(columns - 1);
        ...
        function getStations(){
            ...
            for(var i=0;i<data.length;i++){
                var item = data[i];
                var lat=item[latField],lng=item[lonField],value=item[valueField];
                if (!isNaN(value) && value != null && lat != '' && lng != '' && bounds.contains([lat,lng]))
                    _stations.push( {lat:lat,lng:lng,value:value*valueScale} );
            }
            return _stations;
        }
        ...
        function buildGrid(){
            for (var i = 0; i < rows; i++) {
                for (var j = 0; j < columns; j++) {
                    // var idx = i * columns + j;
                    var item = {gridValue: NaN, valuePoints: [], interpolatePoints: []};
                    grid.push(item);
                }
            }
        }

        function getCell(_row, _column) {
            return grid[_row * columns + _column];
        }

        function getCellLatLng(_row, _column) {
            return L.latLng(bounds.getSouth() + nlat * _row, bounds.getWest() + nlon * _column);
        }

        // 填充各个网格对应的离散点
        function fillValuePoints(){
            for (var i = 0; i < stations.length; i++) {
                var item = stations[i];
                var _row = parseInt((item.lat - bounds.getSouth()) / nlat);
                var _column = parseInt((item.lng - bounds.getWest()) / nlon);
                if (_row < 0 || _column < 0 || _row > rows - 1 || _column > columns - 1)
                    continue;
                var cell = getCell(_row, _column);
        ...
        function idwInterpolate(){
            ...
            for (var i = 0; i < rows; i++) {
                for (var j = 0; j < columns; j++) {
                    ...
                    var cellLatlng = getCellLatLng(i, j);

                    // 查找参与插值的离散点
                    seekIPoints(cell, i - 1, i, j - 1, j);
        ...
        function seekIPoints(cell, startRow, endRow, startColumn, endColumn) {
            //行列范围设定
            startRow = startRow < 0 ? 0 : startRow;
            endRow = endRow > rows - 1 ? (rows - 1) : endRow;
            startColumn = startColumn < 0 ? 0 : startColumn;
            endColumn = endColumn > columns - 1 ? (columns - 1) : endColumn;
            ...
            if (cs * rs <= 4) {
                ...
                for (i = startColumn; i <= endColumn - 1; i++) {
                    iCell = getCell(startRow, i);
                    if (iCell.valuePoints.length > 0)
                        cell.interpolatePoints = cell.interpolatePoints.concat(iCell.valuePoints);
                }
                //左部
                for (i = startRow + 1; i <= endRow; i++) {
                    iCell = getCell(i, startColumn);
                    if (iCell.valuePoints.length > 0)
                        cell.interpolatePoints = cell.interpolatePoints.concat(iCell.valuePoints);
                }
                //右部
                for (i = startRow; i <= endRow - 1; i++) {
                    iCell = getCell(i, endColumn);
                    if (iCell.valuePoints.length > 0)
                        cell.interpolatePoints = cell.interpolatePoints.concat(iCell.valuePoints);
                }
                //下部
                for (i = startColumn + 1; i <= endColumn; i++) {
                    iCell = getCell(endRow, i);
                    if (iCell.valuePoints.length > 0)
                        cell.interpolatePoints = cell.interpolatePoints.concat(iCell.valuePoints);
                }
            }

            if (cell.interpolatePoints.length < iPointMinNum)
                seekIPoints(cell, startRow - 1, endRow + 1, startColumn - 1, endColumn + 1);
            else
                return;
        }
...
(function () {
    ...
    Sun.NCReader = Sun.Class.extend({
        ...
        /**
         * 根据json获取新的nc流
         * @param json {Object} json数据
         * @param uv {Boolean} 是否是uv
         * @returns {Uint8Array}
         */
