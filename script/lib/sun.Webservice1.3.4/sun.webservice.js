...
SW.Ws2_1Invoker = Sun.Class.extend({
    ...
    invoke: function (method, params, version, successFun, errorFun, addToLoading,options) {
        ...
        var data;
        var url = this.getUrl(options, method,version, params);
    ...
    getUrl: function (options, method, version, params) {
        ...
        var url = options.rootURL + "?projectname=&calltype="+options.returntype+"&" + iquery+"&tk="+tk;
        if(options.httpProxyUrl)
            url = options.httpProxyUrl + Sun.Common.encodeBase64(url);
        return url;
    },
...
SW.Ws2_1XHRInvoker = SW.Ws2_1Invoker.extend({
    ...
    invoke: function (method, params, version, successFun, errorFun, addToLoading, options) {
        ...
        this.url = this.getUrl(options, method, version, params);
        var url = type=='GET'?this.url:this.getPostUrl(options, calltype, returntype);
        var debug = options.debug;
        var waitingTip = options.waitingTip;
        var tipMode = options.tipMode;
        if (waitingTip && addToLoading) {
    ...
    getPostUrl: function (options, calltype, returntype) {
        var url = options.rootURL + "?projectname=&calltype="+calltype+"&returntype=" + returntype;
        if(options.httpProxyUrl)
            url = options.httpProxyUrl + Sun.Common.encodeBase64(url);
        return url;
    },
...
SW.Ws3_0_Promise = Sun.Class.extend({
    ...
    getUrl: function (baseParams,params,options) {
        ...
        var url = options.rootURL + "?" + _params;
        if(options.httpProxyUrl)
            url = options.httpProxyUrl + Sun.Common.encodeBase64(url);
        return url;
    ...
    getPostUrl: function (options) {
        var url = options.rootURL + '?calltype=' + this.options.callType;
        if(options.httpProxyUrl)
            url = options.httpProxyUrl + Sun.Common.encodeBase64(url);
        return url;
    },
...
SW.WsRest3_0_Promise = SW.Ws3_0_Promise.extend({
    ...
    getUrl: function (get,method,version,params,options) {
        ...
        _params = _params.slice(1);
        var url = options.rootURL + method + '_v' + version + '?' + _params;
        if(options.httpProxyUrl)
            url = options.httpProxyUrl + Sun.Common.encodeBase64(url);
        return url;
 ...
 *      + v1.3.4 ：增加网络通信代理地址httpProxyUrl的配置，用于http转发，需sun@1.0.23的支持
 *
 * @class Sun.BaseProxy
 */
SW.Rpc.BaseProxy = Sun.Class.extend({
    options: {
        ...
        /**
         * 网络通信代理地址
         * 用于内网webservice只与web服务器相通，不与web访问环境相通的http转发代理
         */
        httpProxyUrl:null,
...
SW.Rpc3_0.BaseProxy = Sun.Class.extend({
     ...
     * @param returnType {String} [optional] 返回类型，如不传则默认为ws的options中的returnType
     * @protected
     * @returns {*}
     */
