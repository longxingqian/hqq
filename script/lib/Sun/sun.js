...
Sun.Util.Color = {
     ...
     * @returns {*}
     */
    colorToRgb : function (sColor, returnType) {
 ...
 *      + v1.0.11 ：changeGridNcToJson支持nc数据version2.0的解析
 *      + v1.0.13 : changeGridNcToJson支持nc站点数据的解析
 *
 * @class Sun.Util.Data
 */
Sun.Util.Data = {
     ...
     * @param isWind {Boolean} 是否是风
     * @param windType {String} 'uv'||'speed'
     * @returns {{}}
     */
    changeGridNcToJson:function (nc,isWind,windType) {
        ...
        data.precision = parseFloat(ncReader.getAttribute('Precision'));
        data.invalidValue = parseFloat(ncReader.getAttribute('MissingValue'));
        data.dataType = ncReader.getAttribute('DataType');
        data.title = ncReader.getAttribute('DataTitle');

        parseFn = data.version=='1.0'?setDataOfVersion1:setDataOfVersion2;
        parseFn();

        return data;
        
        function setAttrOfType3(data) {
            data.stationNum = parseFloat(ncReader.getAttribute('StationNum'));
            data.stationCodes = ncReader.getDataVariable('StationCode');
            // format station codes
            data.stationCodes = data.stationCodes.map(function (code) {
                code = code.toString();
                return code.length<=5 ? code :
                    String.fromCharCode(code.slice(0,code.length-4))+code.slice(code.length-4);
            });
        }
        
        function setAttrOfType4(data) {
            data.latsize = parseFloat(ncReader.getAttribute('LatNum'));
            data.lonsize = parseFloat(ncReader.getAttribute('LonNum'));
            data.startlat = parseFloat(ncReader.getAttribute('StartLat'));
            data.startlon = parseFloat(ncReader.getAttribute('StartLon'));
            data.endlat = parseFloat(ncReader.getAttribute('EndLat'));
            data.endlon = parseFloat(ncReader.getAttribute('EndLon'));
            data.nlat = parseFloat(ncReader.getAttribute('LatInterval'));
            data.nlon = parseFloat(ncReader.getAttribute('LonInterval'));
            data.symbol = ncReader.getAttribute('Symbol');
        }
        
        function setDataOfVersion1() {
            // self attr (Tip:因为许多参数名要兼容之前的json数据参数名，切一些参数需要由字符直接转为float方便之后使用，所以参数列举式赋值)
            var attrFn = data.dataType==3?setAttrOfType3:setAttrOfType4;
            attrFn(data);
            // common attr
            data.units = ncReader.getAttribute('Units');
            data.elementCode = ncReader.getAttribute('ElementCode');
            data.data = getData(data.elementCode,data.precision);
        }
        function setDataOfVersion2() {
            var elementCode = ncReader.getAttribute('ElementCode');
            var eleCodes = elementCode.split(',');
            data.data = {};
            for(var i=0;i<eleCodes.length;i++){
                var code = eleCodes[i];
                var _code = isWind ? code.replace('uv','u') : code;
                var item = {};
                item.latsize = parseFloat(ncReader.getVariableAttribute(_code,'LatNum'));
                item.lonsize = parseFloat(ncReader.getVariableAttribute(_code,'LonNum'));
                item.startlat = parseFloat(ncReader.getVariableAttribute(_code,'StartLat'));
                item.startlon = parseFloat(ncReader.getVariableAttribute(_code,'StartLon'));
                item.endlat = parseFloat(ncReader.getVariableAttribute(_code,'EndLat'));
                item.endlon = parseFloat(ncReader.getVariableAttribute(_code,'EndLon'));
                item.nlat = parseFloat(ncReader.getVariableAttribute(_code,'LatInterval'));
                item.nlon = parseFloat(ncReader.getVariableAttribute(_code,'LonInterval'));
                item.dataLevel = parseInt(ncReader.getVariableAttribute(_code,'DataLevel'));
                item.dataChanged = parseInt(ncReader.getVariableAttribute(_code,'DataChange'));
                item.showLand = parseInt(ncReader.getVariableAttribute(_code,'ShowLand'));
                item.showStyle = parseInt(ncReader.getVariableAttribute(_code,'ShowStyle'));
                item.units = ncReader.getVariableAttribute(_code,'Units');
                item.precision = data.precision;
                item.invalidValue = data.invalidValue;
                item.fixedValue = parseFloat(ncReader.getVariableAttribute(_code,'FixedValue'));
                item.elementCode = code;
                item.data = getData(code,data.precision);
                data.data[code] = item;
            }
        }
...
Sun.Util.LegendData = {
    ...
    getColorOfRangeLegend: function (legendData, value,defaultColor) {
        defaultColor = typeof defaultColor != 'undefined' ? null : '#ffffff';
    ...
    getColorSegments: function (legendData) {
        var segments = [];
        for (var i = 0; i < legendData.length; i++) {
            var item = legendData[i];
            segments.push([item.min, Sun.Util.Color.colorToRgb(item.color, 1)]);
        }
        return segments;
    }
};
